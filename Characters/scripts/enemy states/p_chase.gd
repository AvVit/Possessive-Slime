...
func enter(params: Dictionary = {}):
	...
	pos_ref = get_tree().get_first_node_in_group("possessed")
	if pos_ref:
		diff = pos_ref.global_position.x - enem_ref.global_position.x
		if diff != 0:
			enem_ref.dir.x = sign(diff) * 1

func process(delta : float):
	if(enem_ref.possess_in_attack_range):
		transition_to.emit("attack", self)
		return
	if(!pos_ref):
		enem_ref.possess_in_attack_range = false
		enem_ref.possess_visible = false
